...
func get_next_free_card_slot() -> Node2D:
	var card_slot_parent = self
	var slots = card_slot_parent.get_children()
...
func _input(event):
	if event is InputEventMouseButton:
		var card = deck_reference.raycast_check_for_card()
		if card and event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed and card.uncovered:
				print("uncover")
