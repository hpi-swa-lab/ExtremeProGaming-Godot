...
func _ready() -> void:
	...
	for name in selected_names:
		...
		new_card.get_node("CardBackImage").get_node("BackText").text = card_database_reference.ALL_BUG_CARDS_TEXT[name][1]
		new_card.get_node("CardBackImage").get_node("EffectText").text = card_database_reference.ALL_BUG_CARDS_TEXT[name][2]
