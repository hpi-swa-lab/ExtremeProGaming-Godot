...
func _ready() -> void:
	...
	for name in selected_names:
		...
		new_card.name = name
		new_card.storypoints = card_database_reference.ALL_CARDS_META[name][0]

		var new_card_front_image_path = card_database_reference.ALL_CARDS[name][1]
		var new_card_back_image_path = card_database_reference.ALL_CARDS[name][0]
		...
		new_card.get_node("CardBackImage").scale = Vector2(0.3, 0.28)
